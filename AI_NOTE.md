...
wrote fired on a boring day where the rating dipped by 0.1 point, which
is noise, not a real signal. I only caught that because I read the actual
printed output line by line instead of trusting that the logic was right
because it ran without errors. Tightened the thresholds myself (rating
has to drop by at least 0.3, flag rate has to jump by 50%+) until it only
fired on the real incident. Also hand-checked the numbers in that one
alarm line against the raw CSV rows myself before calling it done.
